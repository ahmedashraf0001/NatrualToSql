...
const forceTerminateProcess = async (process, processName = 'process') => {
    return new Promise((resolve) => {
        ...
        const killProcess = (0, child_process_1.spawn)('taskkill', ['/pid', process.pid.toString(), '/t', '/f'], {
            windowsHide: true,
            stdio: 'pipe'
        });
...
const startApiProcess = () => {
    ...
    try {
        ...
        apiProcess = (0, child_process_1.spawn)('dotnet', [apiPath], {
            cwd: apiDir,
            env: {
                ...process.env,
                'ASPNETCORE_ENVIRONMENT': electron_1.app.isPackaged ? 'Production' : 'Development',
...
electron_1.ipcMain.handle('check-ollama-status', async () => {
    try {
...
electron_1.ipcMain.handle('check-local-llm-health', async () => {
    try {
        const { exec } = require('child_process');
        const { promisify } = require('util');
        const execAsync = promisify(exec);
...
electron_1.ipcMain.handle('install-ollama-setup', async (event) => {
    ...
    try {
        ...
        const sendProgress = (step, message, progress, subProgress, totalSize, cancellable = false, timeEstimate, downloadSpeed) => {
      ...
      `;
        ...
        await new Promise((resolve, reject) => {
            ...
            const updateProgress = (progress, source, details) => {
...
electron_1.ipcMain.handle('cancel-ollama-installation', async () => {
    try {
...
electron_1.ipcMain.handle('start-ollama-model', async () => {
    try {
        const { spawn } = require('child_process');
        const { promisify } = require('util');
        const execAsync = promisify(require('child_process').exec);
...
electron_1.ipcMain.handle('stop-ollama', async () => {
    try {
...
electron_1.ipcMain.handle('check-api-health', async () => {
    try {
...
electron_1.ipcMain.handle('save-groq-api-key', async (event, apiKey) => {
    try {
...
electron_1.ipcMain.handle('get-groq-api-key-status', async () => {
    try {
        let apiDir;
        if (electron_1.app.isPackaged) {
            apiDir = path.join(process.resourcesPath, 'api_publish');
...
electron_1.ipcMain.handle('start-api-process', async (event, setupType) => {
    try {
...
electron_1.ipcMain.handle('stop-api-process', async () => {
    try {
        await stopApiProcess();
...
electron_1.ipcMain.handle('get-api-process-status', async () => {
    return {
